...
const customBudgetOption = "Custom Amount";
...
export default function ClientInquiryForm() {
  ...
  const currentBudgetBrackets =
    currency === "INR" ? inrBudgetBrackets : usdBudgetBrackets;
  ...
  const onSubmit = async (data: ClientInquiryInput) => {
    ...
    try {
      ...
      if (res.success && res.inquiryId) {
        ...
        setServerError(
          res.error ||
            "Failed to submit inquiry. Please review your entries."
        );
      }
    } catch {
      setServerError(
        "A network error occurred while submitting your inquiry. Please try again shortly."
      );
    } finally {
      ...
      <div className="ground-card-dark p-8 md:p-12 text-center space-y-6 max-w-xl mx-auto border border-ground-cream/30">
        ...
        <div className="space-y-2">
          ...
          <p className="text-ground-cream/80 text-sm leading-relaxed font-sans">
            Our ecosystem leads will review your problem scope and contact you
            within 24 to 48 business hours to schedule a Discovery alignment
            call.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-ground-cream/70 font-sans mt-0.5">
              Submit your technical, product, or organizational challenge to be
              built by verified student squads.
            </p>
          </div>

          {/* Currency Toggle */}
          <div className="inline-flex items-center gap-1 p-1 rounded-md bg-[#071521] border border-ground-cream/25 self-start">
            <span className="text-[10px] font-mono text-ground-cream/60 px-1.5 uppercase">
              Currency:
            </span>
            ...
            {currentBudgetBrackets.map((b) => (
              <button
                ...
                className={`p-2.5 rounded border text-xs font-mono text-center transition-all ${
                  selectedBudget === b && !isCustomBudget
